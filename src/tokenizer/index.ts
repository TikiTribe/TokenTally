...
// B12: a real tokenizer and the heuristic should never disagree by more than these factors.
const SANITY_LOW = 0.25;
const SANITY_HIGH = 4;
...
export function countTokens(modelId: string, text: string): TokenCount {
  ...
  if (useAdapter && adapter) {
    try {
      ...
      // B12: reject a plausibly-wrong (non-throwing) adapter count.
      const sane =
        est.count === 0 ? count === 0 : count >= est.count * SANITY_LOW && count <= est.count * SANITY_HIGH;
      if (sane) {
