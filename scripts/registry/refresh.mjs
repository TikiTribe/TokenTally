...
const anchorPrices = (models) =>
  Object.fromEntries(ANCHORS.map((k) => {
    ...
    return [k, m ? `in ${m.inputPrice} / out ${m.outputPrice}` : 'MISSING'];
  }));

const build = readFileSync(BUILD, 'utf8');
const curSha = build.match(/PINNED_COMMIT = '([0-9a-f]{40})'/)[1];
...
// Fetch the exact-commit body (never `main`, so the pin is reproducible) and validate it.
const raw = await (await fetch(`https://raw.githubusercontent.com/${OWNER}/${REPO}/${sha}/${FILE}`, { headers: { 'user-agent': 'tokentally-refresh' } })).text();
JSON.parse(raw);
...
const body = `${headline}\n\n${anchorLine}\n\n<details><summary>${added.length} added</summary>\n\n${list(added)}\n</details>\n\n<details><summary>${removed.length} removed</summary>\n\n${list(removed)}\n</details>\n\nAuto-generated by the monthly \`refresh-pricing\` workflow. Review the model/price deltas above, then merge on green CI.`;
