...
const body = `${headline}\n\n${anchorLine}\n\n<details><summary>${added.length} added</summary>\n\n${list(added)}\n</details>\n\n<details><summary>${removed.length} removed</summary>\n\n${list(removed)}\n</details>\n\nAuto-generated by the monthly \`refresh-pricing\` workflow. Review the model/price deltas above. Merge ONLY once the \`ci\` check is green: that full run (not the pre-PR quick gate) is what runs the hand-computed E2E math oracles, so it is the gate that catches a broken anchor price. The workflow triggers that \`ci\` run for this PR automatically (via workflow_dispatch), so just wait for it.`;
